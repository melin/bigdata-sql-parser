...
class SparkSqlParserTest {
    ...
    fun createTableSelectTest1() {
        val sql = """
               CREATE TABLE t
               PARTITIONED BY (a, b)
               AS SELECT 1 as a, "a" as b, 3 as c
               """
        ...
        if (statement is DcTable) {
            ...
            Assert.assertEquals("SELECT 1 as a, \"a\" as b, 3 as c", statement.querySql)
            Assert.assertEquals("a", statement.partitionColumnNames?.get(0))
            Assert.assertEquals("b", statement.partitionColumnNames?.get(1))
        } else {
            Assert.fail()
        }
    }
